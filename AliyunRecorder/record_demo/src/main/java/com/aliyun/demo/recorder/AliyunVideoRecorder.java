...
//        EncoderDebugger debugger = EncoderDebugger.debug(this, 528, 704);
    ...
    private void toEditor() {
        ...
        final Uri mUri = Uri.fromFile(new File(projectUri.getPath()));
        ...
        fetcher.setParameters(resolutions[0], resolutions[1], AliyunIThumbnailFetcher.CropMode.Mediate, ScaleMode.LB, 1);
        fetcher.requestThumbnailImage(new long[]{0},
                new AliyunIThumbnailFetcher.OnThumbnailCompletion() {
                    ...
                    public void onThumbnailReady(ShareableBitmap frameBitmap, long time) {
                        ...
                        String path = getExternalFilesDir(null) + File.separator + fileName + ".jpeg";
                        try {
                        ...
                        sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE,
                                Uri.parse(mUri.getPath())));
                        Class act = null;
                        try {
                            act = Class.forName("com.aliyun.demo.publish.PublishActivity");
                        } catch (ClassNotFoundException e) {
                            e.printStackTrace();
                        }
                        Intent intent = new Intent(AliyunVideoRecorder.this, act);
                        intent.putExtra("svideo_thumbnail", path);
                        intent.putExtra("project_json_path", mUri.getPath());
                        intent.putExtra("videoDuration", (long) mClipManager.getDuration());
