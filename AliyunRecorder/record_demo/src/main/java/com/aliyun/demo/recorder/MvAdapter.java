...
public class MvAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (viewType == EFFECT_NONE) {
            Glide.with(mContext)
                    .load(R.mipmap.aliyun_video_icon_raw_effect)
                    .into(new ViewTarget<CircularImageView, Drawable>(iMVViewHolder.mImage) {
                        @Override
                        public void onResourceReady(Drawable drawable, Transition<? super Drawable> transition) {
                            iMVViewHolder.mImage.setImageBitmap(((BitmapDrawable) drawable).getBitmap());
