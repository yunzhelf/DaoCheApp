...
public class DiscoverPublishMoodActivity extends BaseActivity {
    ...
    protected int getLayoutId() {
        return R.layout.activity_discover_publish_mood;
    }
    ...
    private void selectPicEvent() {
        PhotoPicker.builder()
                ...
                .start(DiscoverPublishMoodActivity.this, PhotoPicker.REQUEST_CODE);
    }
    ...
    private void publishEvent() {
        mMoodStr = mEt_mood.getText().toString().trim();
        if (TextUtils.isEmpty(mMoodStr)) {
            showToast("请输入这一刻心情");
            return;
        }
        mDialog.show();
...
// task.waitUntilFinished(); // 可以等待任务完成
    ...
    private void requestServer() {
        ...
        RequestBody moodBody = RequestBody.create(MediaType.parse("text/plain"), mMoodStr);
        addUserQuestionMap.put("uId", uIdBody);
        addUserQuestionMap.put("moodBody", moodBody);
        ...
        RxHttpUtils
                ...
                .addShowMood(addUserQuestionMap)
                .compose(Transformer.<AddShowMoodBean>switchSchedulers())
                .subscribe(new CommonObserver<AddShowMoodBean>() {
                    @Override
                    ...
                    @Override
                    protected void onSuccess(AddShowMoodBean response) {
                        closeDialog();
